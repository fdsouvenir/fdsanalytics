...
### 4.3 MCP Server Service Account

**Name:** `mcp-server@fdsanalytics.iam.gserviceaccount.com`
  ...
  template:
    ...
    spec:
      ...
      - image: gcr.io/fdsanalytics/response-engine:latest
        ...
        - name: MCP_SERVER_URL
          value: https://mcp-server-xxxxxxxxxx-uc.a.run.app
```
...
spec:
  template:
    spec:
      serviceAccountName: mcp-server@fdsanalytics.iam.gserviceaccount.com
      containers:
